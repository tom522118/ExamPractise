...
word_list_dict = {}
for i in standard_word_list:
    word_list_dict[i[0]] = i[1]        

        
with open('.\data/new_style_word_list_字根對應單字.pkl', 'rb') as fb:
    new_style_word_dict = pickle.load(fb)
    fb.close()


new_style_word_dict['ath'].remove('math/mathematics')


with open('.\data\可用字根398.pkl', 'rb') as fc:
    root_dict = pickle.load(fc)
    fc.close()
...
class Main(QMainWindow):
    ...
    def restart(self):
        ...
        self.n = random.randint(0,len(root_dict)-1)        
        seed_word = root_dict[self.n]         
        self.word_list = new_style_word_dict[seed_word]        
        word_list = [x for x in self.word_list if x not in kick_out]
        self.chk_quiz()
        self.randomlist = []
        self.QA_list =[]
        ...
        while counter_i <= len(self.radio_buttons):         
            self.n = random.randint(0,len(word_list)-1) 
            if self.n not in self.randomlist:
                self.randomlist.append(self.n)
                counter_i +=1        
        for j in self.randomlist:
            self.QA_list.append(word_list[j])

        self.Answer_int= random.randint(1,len(self.radio_buttons))        
        for i, radio_button in enumerate(self.radio_buttons.values()):
            radio_button.show()
            radio_button.setText(self.QA_list[i])

        qlabel_msg = """<font color='darkred' ><b>{0}</b></font>""".format(word_list_dict[
            self.QA_list[self.Answer_int-1]]
            )
    ...
    def show_msg(self):
        self.msg.setText(self.QA_list[self.Answer_int-1])        
    ...
    def write_to_csv(self):
        log_list = []
        log_list = [self.QA_list[self.Answer_int-1], word_list_dict[self.QA_list[self.Answer_int-1]], self.timer_duration-self.duration,  self.QA_list[self.bg1.checkedId()-1]]
...
if __name__ == '__main__':
    ...
    app.setApplicationDisplayName("中字選英單 特殊字")
    mainWindow = Main()
